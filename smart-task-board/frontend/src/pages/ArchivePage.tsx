...
interface Props { tasks: Task[]; onRestore: (id: string) => void; }
...
export const ArchivePage = ({ tasks, onRestore }: Props) => {
  ...
  return (
    <div className="px-8 py-7">
      <div className="mb-7">
        <h1 className="text-2xl font-bold text-neutral-800 dark:text-cream-100 tracking-tight">Archive</h1>
        <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Completed tasks stored here for reference.</p>
      </div>
      ...
      {archived.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-neutral-300 dark:text-neutral-600 dark:text-neutral-400 gap-3">
          ...
          <p className="text-xs text-neutral-400">Tasks marked as Done will appear here</p>
        </div>
      ) : (
            ...
            {archived.map(task => (
              <div key={task.id} className="group flex items-center justify-between gap-4 px-5 py-4 hover:bg-cream-200 dark:hover:bg-neutral-800 dark:bg-neutral-950 dark:hover:bg-neutral-800/50 transition-colors">
                ...
                <button onClick={() => onRestore(task.id)}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-950/40 hover:bg-primary-100 dark:hover:bg-primary-950/60 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
