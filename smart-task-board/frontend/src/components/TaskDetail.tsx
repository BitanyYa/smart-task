...
export const TaskDetail = ({ task, onClose, onAddComment, onDeleteComment, onToggleTimer }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/25 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-lg max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="px-6 pt-5 pb-3 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100 leading-snug">{task.title}</h2>
            ...
            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0">
              <X size={15} />
            </button>
          </div>
              ...
              {task.comments?.map(c => (
                <div key={c._id} className="flex gap-3 group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 rounded-xl px-3 py-2">{c.text}</p>
                  </div>
                  {user?._id === c.user?._id && (
            ...
            <div className="flex flex-col gap-2">
              ...
              {[...( task.activityLog || [])].reverse().map(a => (
                <div key={a._id} className="flex items-start gap-3 py-2 border-b border-gray-50 dark:border-gray-800 last:border-0">
                  <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-400 shrink-0 mt-0.5">
            ...
            <div className="flex flex-col items-center gap-6 py-4">
              ...
              {task.timeEntries?.length > 0 && (
                <div className="w-full">
                  <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Sessions</p>
                  <div className="flex flex-col gap-1.5">
                    {task.timeEntries.map((e, i) => (
                      <div key={i} className="flex justify-between text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
        ...
        {/* Comment input */}
        {tab === 'comments' && (
          <form onSubmit={handleComment} className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex gap-2">
            <input
              value={comment}
              onChange={e => setComment(e.target.value)}
              placeholder="Write a comment..."
              className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-400/40 focus:border-blue-300 dark:focus:border-blue-600 transition-all text-gray-800 dark:text-gray-200 placeholder-gray-400"
            />
            <button type="submit" className="w-9 h-9 flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors shrink-0">
              <Send size={14} />
            </button>
          </form>
        )}
